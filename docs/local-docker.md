...
- Python パッケージは新規追加せず、バージョン固定済みの `requirements.txt` だけをインストールします。
...
設定検査、Django test、`collectstatic` の dry-run、静的ファイル manifest 参照検査をまとめて実行します。
...
`collectstatic` を dry-run なしで実行すると、bind mount された作業ツリーの `staticfiles/` に出力します。
