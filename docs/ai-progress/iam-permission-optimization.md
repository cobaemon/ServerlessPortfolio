...
`dev` への統合は、[`AGENTS.md`](../AGENTS.md) の `branch-finalize-next` 手順で行います。

ユーザーが検証サイトでの検証、正規手順での作業再開、staging pipeline 検証、または `dev` 反映後の検証を依頼している場合、その依頼は `branch-finalize-next` 実行を責任範囲に含む明示指示として扱います。ローカル commit 後に `branch-finalize-next` 実行前で停止してはいけません。
...
- ローカル検証を実行する。
- 作業ブランチで commit する。
- ユーザーが検証サイトでの検証、正規手順での作業再開、staging pipeline 検証、または `dev` 反映後の検証を依頼している場合、`branch-finalize-next` を実行する。
...
ユーザーが検証サイトでの検証、正規手順での作業再開、staging pipeline 検証、または `dev` 反映後の検証を依頼している場合、作業ブランチを `dev` へ統合します。
...
変更は `vA.B.C` 作業ブランチで commit し、ユーザーが検証サイトでの検証、正規手順での作業再開、staging pipeline 検証、または `dev` 反映後の検証を依頼している場合は `branch-finalize-next` で `dev` へ統合します。
